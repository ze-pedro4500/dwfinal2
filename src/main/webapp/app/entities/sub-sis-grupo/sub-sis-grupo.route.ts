...
export class SubSisGrupoResolve implements Resolve<ISubSisGrupo> {
  constructor(private service: SubSisGrupoService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ISubSisGrupo> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((subSisGrupo: HttpResponse<SubSisGrupo>) => {
          if (subSisGrupo.body) {
            return of(subSisGrupo.body);
          } else {
