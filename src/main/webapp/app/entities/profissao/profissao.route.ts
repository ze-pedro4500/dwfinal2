...
export class ProfissaoResolve implements Resolve<IProfissao> {
  constructor(private service: ProfissaoService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IProfissao> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((profissao: HttpResponse<Profissao>) => {
          if (profissao.body) {
